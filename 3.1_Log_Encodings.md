...
```math
\text{For normalized linear exposure } X \text{ (where } X = 0.18 \text{ represents 18\% gray):}

Y_{\text{linear}} =
\begin{cases}
0 & \text{if } X \leq 0 \\
\frac{64 \times \log_{10}(X) + 512}{1023} & \text{if } X > \frac{0.18}{64} \\
\frac{64 \times \log_{10}(X \times \frac{64}{0.18}) + 512}{1023} & \text{otherwise}
\end{cases}

\text{Encoding to 10-bit code values:}
CV_{\text{10bit}} = \text{round}(Y_{\text{linear}} \times 1023)
```

**Key Characteristics (LogC4 Specification v1.0, 2023):**
- **Dynamic range:** 15+ stops (0.000064 to 1.0 relative exposure)
- **Middle gray placement:** 18% at code value 400/1023 (10-bit) = 0.391 normalized
...
**LogC4 Decoding Equation:**

```math
\text{For 10-bit code value } Y \in [0, 1023]:

X =
\begin{cases}
0 & \text{if } Y \leq 95 \\
10^{\frac{Y \times \frac{1023}{64} - 512}{64}} & \text{if } Y > 95
\end{cases}
```

**Quantization Analysis for LogC4 (10-bit):**

The code value distribution per stop varies logarithmically:

| Stop from 18% | Relative X | Code Value (10-bit) | CV/Stop |
|----------------|------------|---------------------|----------|
| -7 (deep shadow) | 0.0014 | 95 | ~15 |
| -6 | 0.0028 | 159 | ~16 |
| -5 | 0.0056 | 224 | ~16 |
| -4 | 0.0113 | 288 | ~17 |
| -3 | 0.0225 | 352 | ~18 |
...
Y_{\text{S-Log3}} =
\begin{cases}
0.0301342 & \text{if } X \leq 0 \\
5.76 \times X + 0.0301342 & \text{if } 0 < X < 0.01125 \quad \text{(Linear toe)} \\
0.432599 \times \log_{10}(X + 0.084941) + 0.076059 & \text{if } X \geq 0.01125 \quad \text{(Log segment)}
\end{cases}
```
...
- **Middle gray placement:** 18% at code value 410/1023 (10-bit) = 0.401 normalized
...
X =
\begin{cases}
10^{\frac{Y - 0.076059}{0.432599}} - 0.084941 & \text{if } Y > 0.084941 \\
\frac{Y - 0.0301342}{5.76} & \text{if } 0.0301342 < Y \leq 0.084941
\end{cases}
```
...
\begin{cases}
0 & \text{if } X \leq 0 \\
\log_{10}(X \times 10 + 1) \times 0.24 + 0.385 & \text{if } X > 0
\end{cases}
...
- **Middle gray placement:** 18% at code value 450/1023 (10-bit) = 0.44 normalized
